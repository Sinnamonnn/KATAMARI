...
#NOTE: some features are located in vanilla_refresh:other/clock/[number]tick
...
scoreboard players add 2min refresh_count 1

scoreboard players add second refresh_count 1

scoreboard players add 10tick refresh_count 1

scoreboard players add 7tick refresh_count 1

scoreboard players add 2tick refresh_count 1

scoreboard players add 5tick refresh_count 1



execute if score 2min refresh_count matches 2400.. run function vanilla_refresh:other/clock/2400tick

execute if score second refresh_count matches 20.. run function vanilla_refresh:other/clock/20tick

execute if score 10tick refresh_count matches 10.. run function vanilla_refresh:other/clock/10tick

execute if score 2tick refresh_count matches 2.. run function vanilla_refresh:other/clock/2tick

execute if score 5tick refresh_count matches 5.. run function vanilla_refresh:other/clock/5tick

execute if score 7tick refresh_count matches 7.. run scoreboard players set 7tick refresh_count 0
...
execute if score ladder refresh_settings matches 1 as @e[type=marker,tag=refresh_ladder] at @s if entity @p[distance=..32] run function vanilla_refresh:block/ladder/marker
...
execute if score blockanims refresh_settings matches 1 unless score blockanims_beacon refresh_settings matches -1..0 as @e[type=marker,tag=refresh_entity_blockanim_beacon] at @s if entity @p[distance=..96] unless block ~ ~ ~ beacon{Levels:0} run function vanilla_refresh:block/block_anims/beacon/thingies
...
#soul
execute if score gamerule_keepInventory refresh_settings matches 1 if score soul refresh_settings matches 1.. as @e[type=marker,tag=refresh_entity_playersoul] at @s run function vanilla_refresh:death/soul/marker
execute if score gamerule_keepInventory refresh_settings matches 1 if score soul refresh_settings matches 1.. as @e[type=item,tag=refresh_entity_soulitem] unless score @s refresh_item_time matches 600.. at @s run function vanilla_refresh:death/soul/item
...
#invis
execute if score invis refresh_settings matches 1 as @e[type=armor_stand,tag=!refresh_armorstandbypass,tag=!scorched_golem,tag=!scorched_desertgolem] at @s run function vanilla_refresh:entity/invis/root
execute if score invis refresh_settings matches 1 as @e[type=#vanilla_refresh:item_frames] at @s run function vanilla_refresh:entity/invis/root_itemframe
...
execute if score second refresh_count matches 1 run scoreboard players reset @a refresh_player_sneakcount
