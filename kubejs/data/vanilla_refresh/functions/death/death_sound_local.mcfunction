...
execute if score death_sound_local refresh_settings matches 10 run loot spawn ~ ~ ~ loot vanilla_refresh:other/randomize_9
execute if score death_sound_local refresh_settings matches 10 store result score temp refresh_randomizer_output run data get entity @e[distance=..0.01,type=item,limit=1,nbt={Item:{tag:{RefreshRandomizerItem:1}}}] Item.tag.RefreshRandomizerOutput
