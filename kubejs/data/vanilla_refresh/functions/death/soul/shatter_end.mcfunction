...
execute as @s[tag=refresh_entity_playersoul_hasitems] run summon item ~ ~ ~ {Item:{id:"minecraft:bundle",Count:1b},Fire:60}
execute as @s[tag=refresh_entity_playersoul_hasitems] run data modify entity @e[limit=1,sort=nearest,type=item,nbt={Item:{id:"minecraft:bundle"}}] Item.tag.Items set from entity @s data.Inventory
