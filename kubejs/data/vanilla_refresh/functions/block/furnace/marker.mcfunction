execute if score second refresh_count matches 1 unless score @s refresh_count matches 1.. if block ~ ~ ~ blast_furnace[lit=true] if predicate vanilla_refresh:chance/2_percent run scoreboard players set @s refresh_count 100
